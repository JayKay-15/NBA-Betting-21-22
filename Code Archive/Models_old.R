...
for (a in a:g) {
    ...
    knn_margin_scaled <- FNN::knn.reg(knnr_train[,-1], knnr_input, as.numeric(knnr_train$Margin), k=51) #21
    knn_margin <- knn_margin_scaled[["pred"]] * (max(knnr_fit$Margin) - min(knnr_fit$Margin)) + min(knnr_fit$Margin)
    
    knn_win <- predict(knn3(knn_train[,-1], knn_train$Win, k=63), knn_input) #42
...
wb <- loadWorkbook("/Users/Jesse/Documents/MyStuff/NBA Betting/NBA-Betting-21-22/Summary.xlsx")
...
saveWorkbook(wb, file = "/Users/Jesse/Documents/MyStuff/NBA Betting/NBA-Betting-21-22/Summary.xlsx")
